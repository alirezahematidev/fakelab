...
describe("Database", () => {
  test("should initialize with default database options", () => {
    const database = Database.register(new Config({ sourcePath: ["./types"] }));
  ...
  test("should return enabled true when database is enabled", () => {
    const database = Database.register(new Config({ sourcePath: ["./types"], database: { enabled: true } }));
  ...
  test("should return enabled false when database is disabled", () => {
    const database = Database.register(new Config({ sourcePath: ["./types"], database: { enabled: false } }));
